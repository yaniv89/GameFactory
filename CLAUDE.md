...
- **M1 — ECS core and rendering.** Archetype ECS, fixed-step scheduler, PixiJS renderer, AABB collision. Exit: 5000 entities at 60fps reference desktop, 1000 at 60fps Pixel 6a, zero steady-state allocations. Code, tests, and the benchmark harness (`tools/bench/README.md`) are complete; sign-off against the actual Section 18.3 reference devices is blocked on a real device farm, not yet budgeted (see `docs/proposals/0001` Section 6.2) — do not treat this exit criterion as met until that measurement exists.
- **M2 — The sandbox.** ⚠ Highest-risk milestone. QuickJS-in-Worker, bridge surface, capability model, compute budget enforcement. Exit: full `sandbox-escape.test.ts` suite passes, hostile fixtures fail to escape. `sandbox-escape.test.ts` and `capabilityBridge.test.ts` pass (30 tests, real WASM QuickJS, no mocking) — see `docs/security/SANDBOX-DESIGN.md` for what's verified versus still open (a few fixture-checklist items, and the postMessage Worker-hosting wrapper itself, deliberately deferred to M3's real module-loading pipeline since QuickJS-in-WASM's isolation doesn't depend on which JS host runs it). Still required before this milestone is genuinely done, not just internally tested: the external security audit `docs/SPEC.md` Section 21 Risk R3 calls for before Phase 3 launch — nothing in this repo substitutes for that.
...
- **M5 — Backend and persistence.** .NET Minimal API, EF Core migrations, revision commit with optimistic concurrency, authorization policies, rate limiting, security headers, creator accounts and subscription gating (`docs/SPEC.md` Section 23: signup, email verification, login/logout, account/billing page, Free/Pro/Studio plan gates). Exit: every endpoint has a passing cross-tenant 404 test; load test sustains 200 concurrent editors with zero lost writes; a Free-tier workspace gets a `402` with a clear upgrade path on export/publish, and a Pro checkout flips the gate within one Stripe webhook round-trip.
