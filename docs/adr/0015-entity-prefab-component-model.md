...
## Addendum (G2b, #139): reconciling the "once #139 lands" placeholders against what actually shipped

I1b/I1c/I1d landed `Mount`, `Equipment`, and `VfxParticle` as real, registered `ComponentSchema`s in `packages/core/src/components/core.ts` (`registerCoreComponents`) — the exact same mechanism, and the exact same file, as `Health`/`Transform`/`Velocity`/`Sprite`. `MOUNT_PREFAB` (`prefab.ts`) composes `Mount` the way decision 2 above describes. Reconciling against the table above:

- **`vehicle` never became a separate component, and doesn't need to.** A vehicle is a `Mount`-tagged entity with different art (`MOUNT_PREFAB`'s `spriteAssetKey`, decision 4) — the same generalization decision 2's own comment already anticipated ("Adding a new placeable entity type (vehicle, mount, enemy, weapon pickup) becomes 'add a `Prefab` entry'"). No `VehicleFields` type exists or is needed.
- **`mount` shipped as `Mount`** (`riderEntity`, `range`, `mountedMaxSpeed`, `riderBaseMaxSpeed`) — matches decision 2's placeholder in substance, not name.
- **`equippable` shipped as `Equipment`** (`weaponEntity`) — same relationship. `Equipment.weaponEntity` points at a live visual entity, a stated, honest scope boundary (`core.ts`'s own doc comment on the type), not a full inventory-item reference.
- **`vfxEmitter` did not ship**, and this addendum is not adding it speculatively. `VfxParticle` (the *particle*, not an *emitter*) is real and registered; VFX *triggering* remains purely imperative (`spawnVfxBurst`, `vfx.ts`) — a one-shot function called from a damage-event listener, with no entity-attached component marking "this entity is a persistent VFX source" (an always-on torch/aura, as opposed to a one-time hit burst). Nothing in the shipped feature set needs one yet; CLAUDE.md 1's "don't design for hypothetical future requirements" argues against adding it here. If K1's flagship demo (#132) surfaces a real, motivated use (ambient environmental VFX a player actually sees), it gets added there, against that real consumer — not spawned into existence by this reconciliation note.

No code change accompanies this addendum: `Mount` and `Equipment` were already correct, tested, first-class core components before this note was written — G2b's own gap, on inspection, was a stale placeholder-vs-shipped-name mismatch in this ADR's own text, not missing implementation.
