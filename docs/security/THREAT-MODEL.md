...
**A compiled node-graph document (`docs/adr/0017`, J1) is a new input source, not a new trust boundary.** It's project-document data (T6-adjacent, covered by the same "never deserialized as anything other than data" rule this section already states) — a JSON document naming node types and wiring sockets together, never JS/TS source, never `eval`'d or `new Function`'d anywhere on the host. Its *execution* is interpreted entirely inside the existing QuickJS module sandbox (Section 3.1) by a first-party module (`@forge/graph-runtime`) built against the public Module API only — the identical isolation and per-tick compute/memory/stack budget every hand-written module already runs under, whether the graph's author is the project's own creator or, once M4 ships, a third-party module supplying its own node types. No new bridge, no new capability, no new class of code execution this threat model doesn't already cover for module code in general.
