...
describe("PreviewPanel", () => {
  ...
  it("posts the current tile snapshot and this scene's entities once the iframe reports ready", () => {
    ...
    expect(postMessageSpy).toHaveBeenCalledWith({ type: "forge:preview:scene", tiles: [1, 2, 3], entities, installedModules: [], graphs: {}, dataTables: {}, quests: {} }, "*");
  });
  ...
  describe("I1f: dev-preview save/restore bridge", () => {
    it("includes this browser's last dev-preview save in the first scene message, then omits it from later ones", () => {
      ...
      expect(postMessageSpy).toHaveBeenNthCalledWith(1, { type: "forge:preview:scene", tiles: [1, 2, 3], entities: [], installedModules: [], graphs: {}, dataTables: {}, quests: {}, devSave: SAMPLE_SAVE }, "*");

      act(() => useCanvasPreviewStore.setState({ tiles: [4, 5, 6] }));
      expect(postMessageSpy).toHaveBeenNthCalledWith(2, { type: "forge:preview:scene", tiles: [4, 5, 6], entities: [], installedModules: [], graphs: {}, dataTables: {}, quests: {} }, "*");
    });
