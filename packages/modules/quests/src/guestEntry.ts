import { questsModule } from "./index";

declare const __forge_registerModule: (mod: {
  setup: typeof questsModule.setup;
}) => void;
 ...
 * (`dist/guest-bundle.js`). Everything real still lives in `questsModule`
 ...
 * calls `questsModule.setup` directly instead of through the sandbox.
 */
__forge_registerModule({ setup: questsModule.setup });
